...
def leakyReLu(a):
    leaky = np.where(a > 0, a, a*0.01)
    return leaky

def elu(a):
    elu = np.where(a < 0, learning_rate * (np.exp(a) - 1), a)
    return elu
...
def choose_activation_method():
    i = 1
    print("Choose activation method by typing corresponding number:\n")
    print("1. ReLu \n2. LeakyReLu \n3. ELU \n")
    while (i == 1):
        method = int(input())

        if (method != 1 and method != 2 and method != 3):
            print("Type in method 1, 2 or 3 by typing 1, 2 or 3 \n")
            continue
        else:
            break
